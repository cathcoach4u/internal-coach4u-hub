# Lou — Teams Message Stages

Six message templates Lou uses when DMing Cath in Teams. Each one is short, in Cath's voice, and matches the working rhythm in `lou-system-prompt.md`.

Paste each block into the Stages tab of the Lou agent in **Ops > Agents** so they're versioned and editable in the CRM. Use `{{placeholders}}` for dynamic content.
...
- Edit these in **Ops > Agents > Lou > Stages**. The CRM versions every save.
